...
16. `aneel_drp_drc_parquet` e `aneel_dec_fec_parquet` não têm `TransformSpecialist` nem DAG -- falta decidir se o Transform delas é determinístico ou via LLM (ver `projects/ai-energy-data-project/docs/architecture.md`)
17. Persistência real (GCS/BigQuery) -- `LoadGate` hoje usa `gold_writer`/`audit_writer` placeholders que só logam
18. Validação real de ponta a ponta: rede real (ANEEL/Google AI Studio) e Airflow real (`astro dev start`/`astro dev pytest`) -- não foi possível no ambiente de desenvolvimento (sandbox sem saída de rede via `requests`; Docker não estava rodando)
